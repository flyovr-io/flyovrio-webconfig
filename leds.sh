...
function failurestats {
  ...
  AIRCRAFTCOUNT=$(jq '.aircraft_with_pos' /run/flyovrio-feed/status.json)
  ...
  # Failure 4 - no connection to flyovrio
  netstat -apn | grep flyovriofeeder | grep -v 127.0.0.1 >> /dev/null
  if [[ $? -eq 0 ]];
  then
    FAILURES[4]="PASS"
  else
