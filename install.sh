...
if ! [[ -f /boot/flyovrio-version ]]; then
    echo 8.0123456789 > /boot/flyovrio-version
fi
...
for file in helpers/*.sh; do
    echo "www-data ALL = NOPASSWD: /flyovrio/webconfig/$file" >> /etc/sudoers.d/010_www-data
done
